...
const DetailPage = () => {
  ...
  const { EraiNasagerkal } = useParams();

console.log("sdew",EraiNasagerkal)
console.log("categoryDetails",categoryDetails)
  useEffect(() => {
    const getSubContent = async () => {
      try {
        const res = await api.post("/content/getByVappa11", {
          routes:`EraiNasagerkal/${EraiNasagerkal}`,
        });
    ...
    getSubContent();
  }, [EraiNasagerkal]); // Dependency array is empty because `id` is a constant.
