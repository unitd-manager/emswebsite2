...
const DetailPage = () => {
  ...
  const { MahangalDetails } = useParams();

console.log("sdew",MahangalDetails)
console.log("categoryDetails",categoryDetails)
  useEffect(() => {
    const getSubContent = async () => {
      try {
        const res = await api.post("/content/getByVappa11", {
          routes:`MahangalDetails/${MahangalDetails}`,
        });
    ...
    getSubContent();
  }, [MahangalDetails]); // Dependency array is empty because `id` is a constant.
